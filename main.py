...
def mines_start(message):
    user_id = message.from_user.id
    msg = bot.send_message(user_id, "💵 Tikmoqchi bo‘lgan summani kiriting (min: 1000):")
    bot.register_next_step_handler(msg, process_mines_bet)

def process_mines_bet(message):
    user_id = message.from_user.id
    if not message.text.isdigit():
        msg = bot.send_message(user_id, "❌ Faqat raqam kiriting:")
        bot.register_next_step_handler(msg, process_mines_bet)
        return

    bet = int(message.text)
    if bet < 1000:
        msg = bot.send_message(user_id, "❌ Minimal tikish 1000 so‘m.")
        bot.register_next_step_handler(msg, process_mines_bet)
        return

    if user_balances.get(user_id, 0) < bet:
        bot.send_message(user_id, "❌ Balansda yetarli mablag‘ yo‘q.")
        return
    ...
    send_mines_grid(user_id, bombs, [], bet, active=True)

def send_mines_grid(user_id, bombs, opened, bet, active=True, bomb_hit=False, win_amount=0, chat_id=None, msg_id=None):
    buttons = []
    for i in range(25):
        if i in bombs and not active:
            label = "💀"
        elif i in opened:
            label = "💵"
        else:
            label = str(i + 1)
        buttons.append(types.InlineKeyboardButton(label, callback_data=f"mines_{i}"))
...
def handle_cell_click(call):
    ...
    if index in state['mines']:
        state['active'] = False
        send_mines_grid(user_id, state['mines'], state['opened'], state['bet'], active=False, bomb_hit=True,
                        chat_id=call.message.chat.id, msg_id=call.message.message_id)
        return

    send_mines_grid(user_id, state['mines'], state['opened'], state['bet'], active=True,
                    chat_id=call.message.chat.id, msg_id=call.message.message_id)
...
def handle_cashout(call):
    ...
    if opened == 0:
        bot.answer_callback_query(call.id, "❗ Hech narsa ochilmadi.")
        return

    state['active'] = False
    win = int(state['bet'] * (1 + opened * 0.2))
    user_balances[user_id] += win

    send_mines_grid(user_id, state['mines'], state['opened'], state['bet'], active=False, win_amount=win,
                    chat_id=call.message.chat.id, msg_id=call.message.message_id)
